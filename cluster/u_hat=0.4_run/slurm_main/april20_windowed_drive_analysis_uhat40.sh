...
#SBATCH --job-name=april20_uhat40_windowed_drive
#SBATCH --output=april20_uhat40_windowed_drive.txt
#SBATCH --array=1-5
...
BASE_DIR=/home/ikk23
cp $BASE_DIR/underdom/main_scripts/python_driver_windowed.py .
cp $BASE_DIR/underdom/main_scripts/nonWF-model.slim .
cp $BASE_DIR/underdom/main_scripts/slimutil.py .
cp $BASE_DIR/underdom/text_files/april20_windowed_drive_analysis_uhat40.txt .
...
prog=`sed -n "${SLURM_ARRAY_TASK_ID}p" april20_windowed_drive_analysis_uhat40.txt`
