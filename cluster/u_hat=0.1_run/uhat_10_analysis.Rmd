...
```
The a_pred value that minimizes delta is 0.01348485 (only slightly less than the a_pred for u_hat=20). Now, a_pred is an *over-estimate* -- the a_obs value that led to a P(increase) of 50% was 0.01075758. However, this prediction is not as far off as it was for u_hat=20.

The transition range looks like it's between a=0.0075 and a=0.014 (about the same width as for u_hat; around 0.0065).
...
plot_a_vs_delta
```
Again, the a_pred value seems to find the minimum. Again, the grey a_obs line finds a higher delta value, but now it's on the left-hand side of the minimum (a* is an over-estimate).
...
```
Negative correlation this time? That doesn't make sense. If the AUC increases more, we'd expect the drive to be more likely to succeed.
...
plot_delta_vs_freq
```
I think this is because with the range of a I used, we're mainly on the side of the a-vs-delta curve where delta is decreasing as a is increasing (LHS).